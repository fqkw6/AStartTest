...
public class UIDragAndDropable : MonoBehaviour , IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler {
    ...
    // handler
    [SerializeField] string handlerTag = "Player";
    ...
    public void OnEndDrag(PointerEventData d) {            
        ...
        if (dragable && d.button == button) {
            ...
            if (!draggedToSlot) {
                ...
                // specific actions
                foreach (var go in GameObject.FindGameObjectsWithTag(handlerTag))
                    go.SendMessage("OnDragAndClear", this);
            }
        ...
        if (dropable && d.button == button) {
            ...
            if (dropDragable) {
                ...
                if (dropDragable != this) {
                    ...
                    foreach (var go in GameObject.FindGameObjectsWithTag(handlerTag))
                        go.SendMessage("OnDragAndDrop", new UIDragAndDropable[]{dropDragable, this});
                }
            }
        }
    }
